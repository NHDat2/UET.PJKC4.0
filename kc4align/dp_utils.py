...
def print_alignments(alignments, scores=None, mode=None, file=sys.stdout):
    if mode == 'align_sentence':

        list_vis = []
        list_los = []

        if scores is not None:
            for x, s in zip(alignments, scores):
                s_c = s
                if s_c > 0.45:
                    x = x.replace('<==>', '\t')
                    vi = x.split('\t')[0].strip()
                    lo = x.split('\t')[1].strip()
                    mark = False
                    for line in sent_tokenize(vi):
                        if line.strip() in list_vis:
                            mark = True
                    for line in split_sents_lo(lo):
                        if line.strip() in list_los:
                            mark = True
                    if len(vi.split(' ')) > 1 and len(x.split('\t')[-1]) > 2 and 0.5 <= len(clean_text_v2(vi)) / len(clean_text_v2(lo)) <= 1.5 and mark is False:
                        if len(sent_tokenize(vi)) == 1 and len(split_sents_lo(lo)) == 1:
                            list_vis.append(vi)
                            list_los.append(lo)
                            print ('%.4f\t%s' % (s_c, x), file=file)
                        elif len(sent_tokenize(vi)) == 2 and len(split_sents_lo(lo)) == 1 and 0.8 <= len(clean_text_v2(vi)) / len(clean_text_v2(lo)) <= 2.5:
                            list_vis.append(vi)
                            list_los.append(lo)
                            print ('%.4f\t%s' % (s_c, x), file=file)
                        elif len(sent_tokenize(vi)) == 1 and len(split_sents_lo(lo)) == 2 and 0.25 <= len(clean_text_v2(vi)) / len(clean_text_v2(lo)) <= 0.75:
                            list_vis.append(vi)
                            list_los.append(lo)
                            print ('%.4f\t%s' % (s_c, x), file=file)
                        elif len(sent_tokenize(vi)) >= 2 and len(split_sents_lo(lo)) >= 2 and len(sent_tokenize(vi)) != len(split_sents_lo(lo)):
                            list_vis.append(vi)
                            list_los.append(lo)
                            print ('%.4f\t%s' % (s_c, x), file=file)
...
def sentence_segment_vi(lines):
    result = []
    for line in lines:
        line = sent_tokenize(line)
        for l in line:
            # l = clean_text(l).lower()
            if l.strip() != '':
                result.append(l)
    return result


def vecalign(src_lines,
             tgt_lines,
             vecs0,
             vecs1,
    ...
    vis_raw_lines = io.open(vis_raw_file, encoding='utf-8').read().split('\n')
    los_raw_lines = io.open(los_raw_file, encoding='utf-8').read().split('\n')

    final_alignmentsText = []
    for (id_srcs, id_tgts) in stack[0]['final_alignments']:
        ...
        if id_tgts != []:
            ...
            # result_tgt = " ".join([tgt_lines[int(idx)] for idx in id_tgts])
        final_alignmentsText.append(result_src.replace('\n', "") + " <==> " + result_tgt.replace('\n', ""))
    stack[0]['final_alignments_text'] = final_alignmentsText

    for i in range(len(stack[depth]['alignment_scores'])):
        stack[depth]['alignment_scores'][i] = np.absolute(1 - stack[depth]['alignment_scores'][i])
