...
from dp_utils import make_alignment_types, print_alignments, read_alignments, \
    read_in_embeddings, make_doc_embedding, vecalign, sentence_segment_vi, split_sents_lo
...
def _main():
    ...
    parser.add_argument('--mode', type=str, required=True,
                        help='mode align, align for document or sentence')

    parser.add_argument('--los_raw_file', type=str, required=True,
                        help='cxvxc')
    parser.add_argument('--vis_raw_file', type=str, required=True,
                        help='cxvxc')
    ...
    for src_file, tgt_file in zip(args.src, args.tgt):
        ...
        # _src_lines = io.open(src_file, encoding='utf-8').read().strip().split('\n')
        # src_lines = sentence_segment_vi(_src_lines)
        src_lines = io.open(src_file, encoding='utf-8').read().strip().split('\n')

        vecs0 = make_doc_embedding(src_sent2line, src_line_embeddings, src_lines, args.alignment_max_size)

        # _tgt_lines = io.open(tgt_file, encoding='utf-8').read().strip().split('\n')
        # tgt_lines = []
        # for linee in _tgt_lines:
        #     for l in split_sents_lo(linee):
        #         tgt_lines.append(l)
        tgt_lines = io.open(tgt_file, encoding='utf-8').read().strip().split('\n')
        ...
        stack = vecalign(src_lines=src_lines,
                         tgt_lines=tgt_lines,
                         vecs0=vecs0,
                         vecs1=vecs1,
                         ...
                         vis_raw_file=args.vis_raw_file,
                         los_raw_file=args.los_raw_file,)

        # write final alignments to stdout
        # print_alignments(stack[0]['final_alignments'], stack[0]['alignment_scores'], mode=args.mode)
        print_alignments(stack[0]['final_alignments_text'], stack[0]['alignment_scores'], mode=args.mode)
